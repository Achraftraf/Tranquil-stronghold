...
export default function FlyingRocket({ onCatch }: FlyingRocketProps) {
  ...
  const createExplosion = (x: number, y: number) => {
    const explosionStars = Array.from({ length: 20 }, (_, i) => ({
      id: starIdRef.current++,
    ...
    setTimeout(() => {
      setStars(prev => prev.filter(s => !explosionStars.find(es => es.id === s.id)));
    }, 1000);
  };
  ...
  const animate = useCallback(() => {
    ...
    // Enhanced organic movement with more dynamic wobble
    ...
    // More dynamic direction changes with wave patterns
    ...
    // Smooth rotation with slight overshoot for more natural feel
  ...
  const handleCatch = () => {
    if (caught) return; // Prevent double clicks
    
    // Epic catch sequence with genie effect
    setCaught(true);
    setExploding(true);
    ...
    // Trigger game opening with genie effect timing
    setTimeout(() => onCatch(), 800);
  };
    ...
    <>
      ...
      <div
        ...
        className={`fixed z-[100] transition-all duration-300 ease-out will-change-transform ${
          missed ? 'scale-90 opacity-70' : 
          exploding ? 'animate-epic-catch' : 
          'scale-100'
        }`}
        style={{
          left: '50%',
      ...
      >
        ...
        >
          <div className="relative">
            ...
            <div className={`absolute -inset-7 rounded-full bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 blur-xl opacity-60 animate-spin-slow ${exploding ? 'animate-explosion-ring' : ''}`}></div>
            <div className={`absolute -inset-5 rounded-full bg-gradient-to-r from-pink-500/15 via-blue-500/15 to-purple-500/15 blur-lg opacity-50 animate-spin-reverse ${exploding ? 'animate-explosion-ring-2' : ''}`}></div>

            {/* Pulsing glow */}
            <div className={`absolute -inset-4 rounded-full bg-blue-500/10 animate-pulse-glow ${exploding ? 'animate-explosion-flash' : ''}`}></div>
      ...
      <style jsx>{`
        ...
        @keyframes epic-catch {
          ...
          70% { 
            transform: scale(0.6) rotate(540deg) translateY(-30px); 
            opacity: 0.6;
            filter: blur(5px) brightness(1.8);
          }
          85% { 
            transform: scale(0.2) rotate(630deg) translateY(-60px); 
            opacity: 0.3;
            filter: blur(10px) brightness(1.2);
          }
          100% { 
            transform: scale(0.05) rotate(720deg) translateY(-100px); 
            opacity: 0;
        ...
        .animate-epic-catch { 
          animation: epic-catch 0.8s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards; 
          transform-origin: center center;
        }
        .animate-explosion-ring { animation: explosion-ring 0.5s ease-out forwards; }
        .animate-explosion-ring-2 { animation: explosion-ring-2 0.6s ease-out forwards; }
        .animate-explosion-flash { animation: explosion-flash 0.3s ease-in-out 3; }
      `}</style>
    </>
  );
}